...
speed this up by moving our custom app Sass into a separate file. Create and empty file `app.scss` in
...
currently just have a div with some dummy content. In order to render our pages
correctly, we need to replace the `div` with `<BrowserRouter>` (and remove the
text inside).

Now inside `App.js` we'll add the following:
